...
const CreateGradeModal = ({ programmeTypeId }: CreateGradeModalProps) => {
    const [open, setOpen] = useState(false);
    const [title, setTitle] = useState("");
    const [gradeLimit, setGradeLimit] = useState("");
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!programmeTypeId) {
            toast({ title: "Error", description: "No programme type selected", variant: "destructive" });
            return;
        }
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button className="h-11 px-6 rounded-xl font-black tracking-tight text-white shadow-lg shadow-emerald-900/10 transition-all active:scale-95 group">
            ...
            <DialogContent className="sm:max-w-[425px] overflow-hidden border-0 shadow-2xl">
                ...
                <DialogHeader className="mb-4">
                    <DialogTitle className="text-xl font-black text-[#0F3F2A] tracking-tight">Create Grade</DialogTitle>
                    <DialogDescription className="font-medium text-slate-500 text-xs">
                        Define a new grade scale entry for this programme.
                    </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="pt-4 flex justify-end gap-3">
                        ...
                        >
                            {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Create Grade"}
                        </Button>
                    </div>
                </form>
            </DialogContent>
