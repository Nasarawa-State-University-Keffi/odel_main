...
interface UpdateProgrammeModalProps {
    open: boolean;
...
const UpdateProgrammeModal = ({ open, onOpenChange, programmeId }: UpdateProgrammeModalProps) => {
    const { toast } = useToast();
    const queryClient = useQueryClient();

    // Fetch Programme Details
    const { data: programme, isLoading: isLoadingProgramme } = useQuery({
        queryKey: ["programme", programmeId],
        queryFn: () => programmeService.getProgrammeById(programmeId!),
        enabled: !!programmeId && open,
    });
    ...
    useEffect(() => {
        if (programme) {
            setSelectedTypeId(programme.programmeType.id.toString());
        }
    }, [programme]);

    const selectedType = programmeTypes?.find(pt => pt.id.toString() === selectedTypeId);
    ...
    useEffect(() => {
        if (programme) {
            // Determine Level ID (check startingLevel object, level object, or levelId)
            const levelIdStr = programme.startingLevel?.id.toString() ||
                programme.level?.id.toString() ||
                (programme.levelId ? programme.levelId.toString() : "");

            // Determine Award ID
            const awardIdStr = programme.award?.id.toString() ||
                (programme.awardId ? programme.awardId.toString() : ""); // Fallback if no object

            form.reset({
                departmentId: programme.department?.id.toString() || "",
                programmeTypeId: programme.programmeType?.id?.toString() || "",
                name: programme.name || "",
                code: programme.code || "",
                programmeDuration: programme.programmeDuration || 4,
                minimumSemesters: programme.minimumSemesters || 8,
                maximumSemesters: programme.maximumSemesters || 12,
                maximumCapacity: programme.maximumCapacity || 100,
                revenueCode: programme.revenueCode || "",
                levelId: levelIdStr,
                availableOnline: programme.availableOnline ?? true,
                awardId: awardIdStr,
                awardName: programme.award?.name || programme.awardName || "",
            });
            if (programme.programmeType?.id) {
                setSelectedTypeId(programme.programmeType.id.toString());
            }
        }
    }, [programme, form]);

    // Force re-validation when programmeTypeId changes
    useEffect(() => {
        form.trigger("levelId");
    }, [selectedTypeId, form, isLevelRequired]);
    ...
    const { data: levels, isLoading: isLoadingLevels } = useQuery({
        queryKey: ["levels", selectedTypeId],
        queryFn: () => levelService.getAllLevels(Number(selectedTypeId)),
    ...
    const updateMutation = useMutation({
        mutationFn: (values: z.infer<typeof schema>) => {
            return programmeService.updateProgramme(programmeId!, {
                ...values,
                departmentId: Number(values.departmentId),
                programmeTypeId: Number(values.programmeTypeId),
                levelId: values.levelId ? Number(values.levelId) : 0,
        ...
        onError: (error: any) => {
            ...
            let message = "Failed to update programme.";
    ...
    // Handle Programme Type Change manually to update state
    const handleTypeChange = (typeId: string) => {
        setSelectedTypeId(typeId);
        form.setValue("programmeTypeId", typeId);
        form.setValue("levelId", ""); // Reset level when type changes
    };
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Edit the details of the academic programme.
                    </DialogDescription>
                </DialogHeader>
...
export default UpdateProgrammeModal;
