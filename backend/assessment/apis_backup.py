...
@extend_schema_view(
    list=extend_schema(
        summary="List my assignment submissions (students)",
    ...
    retrieve=extend_schema(
        summary="Retrieve my submission",
...
class StudentAssignmentSubmissionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    Students can only view, start, and submit their own submissions.
    """
    ...
    def get_queryset(self):
        ...
        # Get student_external_id from request (could be from JWT, session, or user model)
        student_id = self.request.user.username  # Adjust based on your auth setup
    ...
    def start(self, request):
        ...
        student_id = request.user.username  # Get from authenticated user
...
@extend_schema_view(
    list=extend_schema(
        summary="List all assignment submissions (staff)",
    ...
    retrieve=extend_schema(
        summary="Retrieve any submission (staff)",
...
class StaffAssignmentSubmissionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    Staff can view all submissions, filter, and grade them.
    """
    serializer_class = AssignmentSubmissionSerializer
...
# Keep old ViewSet for backward compatibility (deprecated)
@extend_schema_view(
...
class AssignmentSubmissionViewSet(viewsets.ReadOnlyModelViewSet):
    """
    DEPRECATED: Use StudentAssignmentSubmissionViewSet or StaffAssignmentSubmissionViewSet instead.
    ...
    def get_queryset(self):
        return AssignmentSubmission.objects.all().select_related('assignment')
