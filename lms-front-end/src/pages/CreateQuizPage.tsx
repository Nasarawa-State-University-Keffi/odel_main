...
const CreateQuizPage = () => {
    ...
    const { createQuiz, isPending } = useStaffQuiz();
    const { fetchStaffData, isPending: pendingStaff, dashboardData } = useStaffDashboard()
    const { user } = useUserContext()



    // Form data state matching the CreateQuizPayload exactly
    ...
    const handleToggle = (name: keyof CreateQuizPayload) => {
        setFormData((prev) => ({
            ...prev,
            [name]: !prev[name as keyof CreateQuizPayload],
        }));
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Convert local input dates to standard ISO strings before sending
            const payload: CreateQuizPayload = {
                ...formData,
                time_open: new Date(formData.time_open).toISOString(),
                time_close: new Date(formData.time_close).toISOString(),
            ...
            toast.error(typeof err === "string" ? err : "Failed to create quiz.");
        }
    };
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs sm:text-sm text-zinc-500 dark:text-zinc-400">
                                Configure settings, timing, and rules for a new assessment.
                            </p>
                        </div>
                    </div>
                        ...
                        <AnimateIn direction="up" className="bg-white dark:bg-zinc-950 p-5 sm:p-7 rounded-2xl border border-zinc-200 dark:border-zinc-800/80 shadow-sm flex flex-col gap-6">
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="relative">
                                    ...
                                    <select
                                        ...
                                        className="w-full pl-10 pr-4 h-11 rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all appearance-none disabled:opacity-50"
                                    >
                                        <option value="" disabled>
                                            {pendingStaff ? "Loading courses..." : "Select a course"}
                                        </option>
                                        {dashboardData?.total_courses?.map((course: any) => (
                                            <option key={course.course_external_id} value={course.course_external_id}>
                                                {course.course_title}
                                            </option>
                                        ))}
                                    </select>
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <input
                                    ...
                                    value={formData.name}
                                    onChange={handleChange}
                                    placeholder="e.g. Midterm General Knowledge Quiz"
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <textarea
                                    ...
                                    onChange={handleChange}
                                    placeholder="Provide clear instructions for your students before they begin..."
                                    className="w-full p-4 rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all resize-y min-h-[120px]"
                                />
                            </div>
                        </AnimateIn>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div className="flex flex-col gap-2">
                                    <label className="text-xs font-semibold text-zinc-700 dark:text-zinc-300">
                                        Open Quiz At <span className="text-red-500">*</span>
                                    </label>
                                    <input
                                        type="datetime-local"
                                        name="time_open"
                                ...
                                <div className="flex flex-col gap-2">
                                    <label className="text-xs font-semibold text-zinc-700 dark:text-zinc-300">
                                        Close Quiz At <span className="text-red-500">*</span>
                                    </label>
                                    <input
                                        type="datetime-local"
                                        name="time_close"
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <AnimateIn direction="up" delay={0.2} className="bg-white dark:bg-zinc-950 p-5 sm:p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800/80 shadow-sm flex flex-col gap-5">
                            ...
                            <div className="flex items-center justify-between gap-4">
                                ...
                                <button
                                    type="button"
                                    onClick={() => handleToggle("shuffle_questions")}
                                    className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${formData.shuffle_questions ? "bg-emerald-500" : "bg-zinc-200 dark:bg-zinc-800"
                                        }`}
                                >
                                    <span
                                        className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-lg ring-0 transition duration-200 ease-in-out ${formData.shuffle_questions ? "translate-x-5" : "translate-x-0"
                                            }`}
                                    />
                                </button>
                            </div>
                            ...
                            <div className="flex items-center justify-between gap-4">
                                ...
                                <button
                                    type="button"
                                    onClick={() => handleToggle("show_feedback")}
                                    className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${formData.show_feedback ? "bg-emerald-500" : "bg-zinc-200 dark:bg-zinc-800"
                                        }`}
                                >
                                    <span
                                        className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-lg ring-0 transition duration-200 ease-in-out ${formData.show_feedback ? "translate-x-5" : "translate-x-0"
                                            }`}
                                    />
                                </button>
                            </div>
