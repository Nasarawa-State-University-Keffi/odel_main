...
import type { QuestionPayload } from "@/types/questionBank.types";
...
export const StaffEditQuestionPage: React.FC = () => {
    const { id } = useParams<{ id: string }>();
    const navigate = useNavigate();
    const { fetchQuestionById, updateQuestion, fetchCategories, isLoading: isActionLoading } = useStaffQuestionBank();
    ...
    // Fetch categories and question details on mount
    useEffect(() => {
        const loadInitialData = async () => {
            try {
                const [catResponse, questionResponse] = await Promise.all([
                    fetchCategories(),
                    fetchQuestionById(id!)
                ]);

                // Handle categories list extraction
                const catList = Array.isArray(catResponse)
                    ? catResponse
                    : catResponse?.results || [];
                setCategories(catList);

                // Handle question data population
                if (questionResponse) {
                    ...
                    setFormData({
                        ...
                        default_mark: String(q.default_mark ?? "1"),
                        penalty: String(q.penalty ?? "0.33"),
                    });

                    if (Array.isArray(q.answers) && q.answers.length > 0) {
                        setAnswers(q.answers);
                    } else {
                        // Fallback defaults if none returned
                        setAnswers([
                ...
                setErrorMessage("Failed to load question details. Please check your connection.");
            } finally {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await updateQuestion(id!, payload as QuestionPayload);
            ...
            if (typeof err === "object" && err !== null) {
                ...
                setErrorMessage(typeof err === "string" ? err : "Failed to update question.");
            }
        }
    };
    ...
    if (isLoadingData) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3 text-zinc-400">
                ...
                <p className="text-sm font-medium">Loading question details...</p>
            </div>
        );
    }
        ...
        <main className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
                            Modify assessment parameters, content, and scoring criteria.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <AnimateIn direction="up">
                <form onSubmit={handleSubmit} className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800/80 rounded-2xl shadow-sm p-6 sm:p-8 flex flex-col gap-8">
                    ...
                    {/* Section 3: Conditional Answer Options */}
                    <div className="flex flex-col gap-5 pt-4">
...
export default StaffEditQuestionPage;